...
// udp报文结构，当接收到udp报文时，会用这个结构来保存udp报文数据，并插入到接收链表中。
...
func (e *endpoint) Write(p tcpip.Payload, opts tcpip.WriteOptions) (uintptr, <-chan struct{}, *tcpip.Error) {
	...
	var dstPort uint16
	if to == nil { // 如果没有指定发送的地址，用UDP端 Connect 得到的路由和目的端口
		route = &e.route
		dstPort = e.dstPort
...
// Bind 将该UDP端绑定本地的一个IP+端口(网卡是可选的)
...
// Connect 连接UDP对端，当发送数据的时候用这个地址来填充目标地址
