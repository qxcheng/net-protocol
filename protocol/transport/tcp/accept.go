...
// listenContext 监听端点在监听连接时用于存储相关状态，其成员和方法不能并发
...
// incSynRcvdCount 尝试增加处于 SYN-RCVD 状态的端点的计数
...
func newListenContext(stack *stack.Stack, rcvWnd seqnum.Size, v6only bool,
	netProto tcpip.NetworkProtocolNumber) *listenContext {
...
// createCookie 根据id和序列号创建 SYN cookie
...
func (l *listenContext) createConnectedEndpoint(s *segment, iss seqnum.Value, irs seqnum.Value,
	rcvdSynOpts *header.TCPSynOptions) (*endpoint, *tcpip.Error) {
...
// createEndpoint 创建连接状态的新端点，然后执行三次握手
