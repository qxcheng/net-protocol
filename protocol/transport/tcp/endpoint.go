...
import (
	...
	"github.com/qxcheng/net-protocol/pkg/tmutex"
...
type endpoint struct {
	...
	lastError   *tcpip.Error  // 端点上次报告的错误

	rcvListMu  sync.Mutex
	rcvList    segmentList   // 存储ready-for-delivery包，可由Read()读取
	rcvClosed  bool          // 对端关闭发送方时，设为true表明没有更多的数据了
	rcvBufSize int
	rcvBufUsed int

	// 以下字段被锁保护
	mu                sync.RWMutex
	id                stack.TransportEndpointID  // 端点的唯一标识
	state             endpointState
	isPortReserved    bool                       // 该端点是否注册了端口
	isRegistered      bool                       // 该端点是否已注册到分流器
	boundNICID        tcpip.NICID                // 端点绑定的网卡ID
	route             stack.Route                // 端点的路由
	v6only            bool
	...
	hardError *tcpip.Error // 在stateError状态下进行读写返回的错误，被mu保护
	workerRunning bool     // 工作线程是否正在运行
	...
	recentTS uint32
	tsOffset uint32  // 一个在 timestamp 选项中的 TSVal 字段添加的随机偏移量

	shutdownFlags tcpip.ShutdownFlags  // 表示当前的关闭状态

	sackPermitted bool  // 设为true，如果对端在SYN/SYN-ACK报文中发送了TCPSACKPermitted选项
	sack SACKInfo       // 持有该端点的 TCP SACK 信息
	...
	segmentQueue segmentQueue // 包的接收队列

	sndBufMu      sync.Mutex
	sndBufSize    int
	sndBufUsed    int
	sndClosed     bool         // 发送端关闭时设为true
	sndBufInQueue seqnum.Size
	sndQueue      segmentList  // 发送队列
	sndWaker      sleep.Waker  // 唤醒发送func
	sndCloseWaker sleep.Waker  // 发送端关闭时用于唤醒

	cc CongestionControlOption  // 拥塞算法名
	...
	newSegmentWaker sleep.Waker    // 有新的包到来时唤醒
	notificationWaker sleep.Waker  // 有新的通知到来时唤醒
	notifyFlags uint32             // 通知类型的位图，需要原子方式访问
...
// HandlePacket 从网络层接收到该端点的数据，会调用该函数来处理报文
...
// readyToRead 被调用，当一个新的segment准备好被读取，或连接的接收端关闭时（此时s为nil）
...
// updateSndBufferUsage 当收到ack确认时，需要更新发送缓冲占用，即新增空间为v
