...
const (
	maxOptionSize = 40  // tcp选项的最大长度
)

// tcp三次握手时候使用的对象
type handshake struct {
	ep *endpoint
	state  handshakeState  // 握手的状态
	active bool
	flags  uint8
	ackNum seqnum.Value
	iss seqnum.Value    // 初始序列号
	rcvWnd seqnum.Size  // 接收窗口
	sndWnd seqnum.Size  // 发送窗口
	mss uint16          // 最大报文段大小
	sndWndScale int     // 发送窗口扩展因子 负数表示对端不支持
	rcvWndScale int     // 接收窗口扩展因子
}
...
// FindWndScale 对于给定的最大窗口大小，确定窗口扩展因子
...
// synRcvdState 正常情况下，会调用该函数来处理第三次 ack 报文
...
func sendSynTCP(
	r *stack.Route, id stack.TransportEndpointID, flags byte, seq, ack seqnum.Value,
	rcvWnd seqnum.Size, opts header.TCPSynOptions) *tcpip.Error {
...
func (e *endpoint) sendRaw(data buffer.VectorisedView, flags byte, seq, ack seqnum.Value,
	rcvWnd seqnum.Size) *tcpip.Error {
...
func sendTCP(r *stack.Route, id stack.TransportEndpointID, data buffer.VectorisedView,
	ttl uint8, flags byte, seq, ack seqnum.Value, rcvWnd seqnum.Size, opts []byte) *tcpip.Error {
...
// handleSegments 从队列中取出 tcp 段数据并处理
