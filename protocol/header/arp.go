...
const (
	// ARPProtocolNumber ARP协议号，为0x0806
	ARPProtocolNumber tcpip.NetworkProtocolNumber = 0x0806

	// ARPSize ARP报文在IPV4网络下的长度
...
// ProtocolAddressSender 从报文中得到arp发送方的协议地址，为ipv4地址
func (a ARP) ProtocolAddressSender() []byte {
	const s = 8 + 6
	return a[s : s+4]
}

// HardwareAddressTarget 从报文中得到arp目的方的硬件地址
func (a ARP) HardwareAddressTarget() []byte {
	const s = 8 + 6 + 4
	return a[s : s+6]
}

// ProtocolAddressTarget 从报文中得到arp目的方的协议地址，为ipv4地址
